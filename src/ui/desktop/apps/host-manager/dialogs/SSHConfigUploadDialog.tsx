...
function extractIdentityFiles(configText: string): string[] {
  ...
  for (const line of configText.split(/\r?\n/)) {
    ...
    const match = trimmed.match(/^identityfile\s*[=\s]\s*(.+)/i);
    if (match) {
    ...
    >
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload your local <code>~/.ssh/config</code> file and any referenced
            private key files. All hosts will be imported.
          </DialogDescription>
        </DialogHeader>
        ...
        {identityFiles.length > 0 && step !== "config" && (
          <>
            ...
            <p className="text-xs text-muted-foreground mb-2">
              Upload the private key files referenced in your config. Skip any
              you don't have — those hosts will be imported without key auth and
              you can add keys later.
            </p>
