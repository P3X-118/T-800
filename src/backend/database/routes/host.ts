  ...
  async (req: Request, res: Response) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        credentialErrors.push(`${entry.name}: failed to create credential`);
        unreadablePairs.add(key);
      }
    }
        ...
        const sshDataObj: Record<string, unknown> = {
          ...
          notes: (() => {
            ...
            if (entry.dynamicForwards.length > 0) {
              parts.push(`DynamicForward: ${entry.dynamicForwards.join(", ")}`);
            }
  ...
  async (req: Request, res: Response) => {
    ...
    for (const entry of body.keys) {
      if (
        ...
        errors.push(
          `Invalid key entry for ${entry?.identityFile || "unknown"}`,
        );
        continue;
      }
